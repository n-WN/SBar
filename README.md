...
| 构建产物 | DMG 包体积 | App 体积（解包后） |
| --- | ---: | ---: |
| Apple Silicon（内置 Core） | 13.9 MB | 18 M |
| Apple Silicon（无内核） | 2.6 MB | 7.3 M |
| Intel（内置 Core） | 14.8 MB | 19 M |
| Intel（无内核） | 2.7 MB | 7.4 M |

> [!NOTE]
> 数据来自 GitHub Releases `v0.2.0` 的实际产物体积（DMG 为下载大小，App 为 DMG 内解包后的大小），不同版本与构建方式会略有波动。
